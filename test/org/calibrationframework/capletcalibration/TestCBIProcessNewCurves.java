...
import org.calibrationframework.montecarlo.models.MonteCarloCBIDrivenMultiCurveModel;
import org.calibrationframework.montecarlo.process.MonteCarloFlowOfTemperedCBIProcess;
import org.calibrationframework.montecarlo.products.MonteCarloMultiCurveCapletPricer;
...
public class TestCBIProcessNewCurves {
	...
	public static void main(String[] args) throws SolverException, CloneNotSupportedException, CalculationException {	
		...
		*/
		...
		double timeHorizon = 6.0;
		int numberOfTimeSteps = 200;
		
		CBIDrivenMultiCurveModel oldModel = new CBIDrivenMultiCurveModel(timeHorizon, numberOfTimeSteps, calibratedModel, tenors, initialValues, immigrationRates, b, sigma, eta, zeta, alpha, lambda);
		...
		ScalarParameterInformationInterface alphaInfo = new ScalarParameterInformation(true, new BoundConstraint(1.01, 1.91));
		
		ScalarParameterInformationInterface[] initialValuesInfo = 
			{new ScalarParameterInformation(true, new BoundConstraint(1E-5, 1E-2)),
					new ScalarParameterInformation(true, new BoundConstraint(1E-5, 1E-2))};
		...
		for(int i = 20; i < 45; i++) {
			...
			//QuantizationMultiCurveSmileOfCaplets myCapletQunt = new QuantizationMultiCurveSmileOfCaplets("forward-EUR-6M", maturity, strikes);
