...
import org.calibrationframework.marketdata.model.volatilities.CapletSmileData;
import org.calibrationframework.marketdata.model.volatilities.CapletSurfaceData;
...
public class CapletCalibrationFFTtest {
	
	public static void main(String[] args) throws SolverException, CloneNotSupportedException, SolverException, org.calibrationframework.optimizer.SolverException {
		...
		for(int i=0; i<maturities.length; i++) {
			...
			if(time < 2.0) {
				tenor1 = "forward-EUR-3M";
				tenor2 = 0.25;
			} else {
				tenor1 = "forward-EUR-6M";
				tenor2 = 0.5;
			}
			
			for(int j=0; j<strikes.length; j++) {
				
				prices[i][j] = net.finmath.functions.AnalyticFormulas.bachelierOptionValue(calibratedModel.getForwardCurve(tenor1).getValue(time-tenor2), volatilities[i][j], time, strikes[j], tenor2*calibratedModel.getDiscountCurve("discount-EUR-OIS").getDiscountFactor(time));
		...
		//Target caplet data
		QuotingConvention convention = QuotingConvention.PRICE;
		...
		for(int i = 0; i<indeces.length; i++) {
			...
			for(int j = 0; j <strikeIndeces.length; j++) {
				values[j] = prices[index][strikeIndeces[j]];
			}
		...
		CapletCalibrationProblem problem = new CapletCalibrationProblem(surface, model, optimizerFactory, pricer,initialParameters,parameterStep,CapletCalibrationProblem.Error.ARPE);
