...
import org.calibrationframework.fouriermethod.calibration.deeplearning.DeepApproximation;
...
public class TestDeepApproximation {
	...
	public void test() {
		...
		DeepApproximation deepPricing = new DeepApproximation(model, weightedSurfaces, 
				new NormalizerMinMaxScaler(), DeepApproximation.Data.ALLSURFACES);
		
		int miniBatchSizeTrainingSet = 32;
		int miniBatchSizeTestingSet = 20;
		double ratioTrainTest = 0.8;
		int numEpochs = 150;
		int numHiddenLayers = 3;
		
		System.out.println("nIn = " + deepPricing.getInputSizes()[0]);
		System.out.println("nOut = " + deepPricing.getOutputSizes()[0]);
		
		System.out.println("Deep learning approximation starting point");
		
		long startMillis	= System.currentTimeMillis();
		try {
			deepPricing.runDeepLearningApproximation(miniBatchSizeTrainingSet, 
					miniBatchSizeTestingSet, ratioTrainTest, numEpochs, numHiddenLayers, true);
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		System.out.println("Deep learning approximation completed in: " +calculationTime + " seconds");
