...
import org.calibrationframework.fouriermethod.calibration.deeplearning.*;
import org.calibrationframework.fouriermethod.calibration.models.TemperedStableCBITCLMultiCurrencyModel;
...
public class TestDeepCalibration {
	...
	public void test() {
		...
		WeightedOptionSmileData[] weightedSmilesEURUSD = new WeightedOptionSmileData[maturities.length];
		WeightedOptionSmileData[] weightedSmilesEURJPY = new WeightedOptionSmileData[maturities.length];
		WeightedOptionSmileData[] weightedSmilesUSDJPY = new WeightedOptionSmileData[maturities.length];
		...
		for(int i = 0; i< maturities.length; i++) {
			...
			weightedSmilesEURUSD[i] = new WeightedOptionSmileData("EURUSD", referenceDate, strikesEURUSD[i], maturity, volEURUSD[i], weightEURUSD[i], convention);
			weightedSmilesEURJPY[i] = new WeightedOptionSmileData("EURJPY", referenceDate, strikesEURJPY[i], maturity, volEURJPY[i], weightEURJPY[i], convention);
			weightedSmilesUSDJPY[i] = new WeightedOptionSmileData("USDJPY", referenceDate, strikesUSDJPY[i], maturity, volUSDJPY[i], weightUSDJPY[i], convention);
			
		}
		
		WeightedOptionSurfaceData weightedSurfaceEURUSD = new WeightedOptionSurfaceData(weightedSmilesEURUSD, discountCurveUSD, forwardCurveEURUSD);
		WeightedOptionSurfaceData weightedSurfaceEURJPY = new WeightedOptionSurfaceData(weightedSmilesEURJPY, discountCurveJPY, forwardCurveEURJPY);
		WeightedOptionSurfaceData weightedSurfaceUSDJPY = new WeightedOptionSurfaceData(weightedSmilesUSDJPY, discountCurveJPY, forwardCurveUSDJPY);
		
		LinkedHashMap<String, WeightedOptionSurfaceData> weightedSurfaces = new LinkedHashMap<String, WeightedOptionSurfaceData>();
		weightedSurfaces.put("EURUSD", weightedSurfaceEURUSD);
		weightedSurfaces.put("EURJPY", weightedSurfaceEURJPY);
		weightedSurfaces.put("USDJPY", weightedSurfaceUSDJPY);
		...
		TemperedStableCBITCBrownian[] temperedStableCBITCLs = new TemperedStableCBITCBrownian[d];
		
		for(int k = 0; k < d; k++) {
			temperedStableCBITCLs[k] =  new TemperedStableCBITCBrownian(timeHorizon, numberOfTimeSteps, 
		 ...
		 * Optimizer factory
		 */
		OptimizerFactoryInterface optimizerFactory = new OptimizerFactoryLevenbergMarquardt(LevenbergMarquardt.RegularizationMethod.LEVENBERG,
				300 /* maxIterations */, 1e-3 /* Error Tolerance */, 4 /* maxThreads */); 
		//new OptimizerFactoryCMAES(1e-2, 300);
		
		DeepCalibrationProblem deepCalibration = null;
		try {
			deepCalibration = new DeepCalibrationProblem(model, weightedSurfaces, optimizerFactory,
					DeepApproximation.Data.ALLSURFACES);
		...
		DeepCalibrationProblem.OptimizationResult result = null;
		try {
			result = deepCalibration.runCalibration();
		...
		ArrayList<String> comparisonMarketNeuralNetwork = result.getComparisonMarketNeuralNetwork();
		
		for(String myString : comparisonMarketNeuralNetwork)
			System.out.println(myString);
		
		double[] calibratedParameters = model.getCloneForModifiedParameters(parameters).getParameters();
		for(int i = 0; i<calibratedParameters.length; i++) 
			System.out.println(calibratedParameters[i]);
