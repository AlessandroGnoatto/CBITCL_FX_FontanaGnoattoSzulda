...
public abstract class LevenbergMarquardt implements Serializable, Cloneable, OptimizerInterface {
	...
	public void setDerivatives(double[] parameters, double[][] derivatives) throws SolverException {
		...
		Vector<Future<double[]>> valueFutures = new Vector<>(parameterCurrent.length);
	...
	public double getMeanSquaredError(double[] value) {
		...
		for (int valueIndex = 0; valueIndex < value.length; valueIndex++) {
			...
			error += weights[valueIndex] * weights[valueIndex] * deviation * deviation;
		}
		...
		while (hessianInvalid) {
			...
			for (int i = 0; i < parameterCurrent.length; i++) {
				for (int j = i; j < parameterCurrent.length; j++) {
					...
					for (int valueIndex = 0; valueIndex < valueCurrent.length; valueIndex++) {
						alphaElement += weights[valueIndex] * weights[valueIndex] * derivativeCurrent[i][valueIndex] * derivativeCurrent[j][valueIndex];
					}
					if (i == j) {
