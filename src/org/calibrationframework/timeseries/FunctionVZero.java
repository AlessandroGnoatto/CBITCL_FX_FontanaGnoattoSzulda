...
public class FunctionVZero implements TimeSeriesInterface {
	...
	public FunctionVZero(double timeHorizon, int numberOfTimeSteps, double lambda, DoubleUnaryOperator psi) {
		this.lambda = lambda;
		this.psi = psi;
			...
			public Iterator<Double> iterator() {
				return new Iterator<Double>() {
					...
					public boolean hasNext() {
						return index < FunctionVZero.this.getNumberOfTimePoints();
					}

					@Override
					public Double next() {
						return FunctionVZero.this.getValue(index++);
					}
				};
			}
